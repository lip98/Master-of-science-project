...
while (OVER==0)
    ...
    %we execute the move
    temp = Init(XY1(2),XY1(1));
    Init(XY1(2),XY1(1))= "-";
    Init(XY2(2),XY2(1))= temp;
    boardCurrent = showBoardCurrent(Init,board);
    imshow(boardCurrent);
    %%here we will send the coordinates of the chess piece to the robotic
    %%hand
    %then we will wait for the response from the chess engine, for now it
    %is manual
